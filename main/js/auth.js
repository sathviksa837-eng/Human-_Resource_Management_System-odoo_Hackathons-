...
export class Auth {
  ...
  static login(emailOrId, password) {
    ...
    if (!target || !pass) {
      throw new Error('Please enter both Login ID/Email and Password.');
    }
    
    const user = users.find(u => 
      u &&
      ((u.email && u.email.toLowerCase() === target) || (u.id && u.id.toLowerCase() === target)) && 
      u.password && u.password.trim() === pass
    );
