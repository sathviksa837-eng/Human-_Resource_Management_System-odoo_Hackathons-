...
import { db, generateEmployeeID, formatTime12h, calculateWorkHours } from './db.js';
...
class App {
  ...
  static init() {
    ...
    window.AppOpenView = (v, p) => App.openView(v, p);
    Auth.init();
  ...
  static promptPortalAuth(mode) {
    ...
    const modeInput = document.getElementById('portal-auth-mode');
    const idInput = document.getElementById('portal-auth-id');
  ...
  static updateUserHeader(user) {
    ...
    // Live Attendance Status Indicator Dot & Menu State (Strict 1-time per day restriction)
    ...
    if (statusDot) {
      if (isCheckedIn) {
        ...
        statusDot.style.boxShadow = '0 0 10px rgba(59, 130, 246, 0.8)';
        statusDot.title = 'Attendance Status: Completed for Today';
      } else {
    ...
    if (statusLabel) {
      if (isCheckedIn) {
        statusLabel.innerHTML = '<span style="color: #10b981; font-weight: 700;">🟢 Checked IN</span>';
      } else if (isCompletedToday) {
        statusLabel.innerHTML = '<span style="color: #3b82f6; font-weight: 700;">🔵 Completed Today</span>';
      } else {
        statusLabel.innerHTML = '<span style="color: #ef4444; font-weight: 700;">🔴 Checked Out</span>';
      }
    }

    if (checkinText) {
      if (isCheckedIn) {
        checkinText.innerText = 'Check OUT →';
      } else if (isCompletedToday) {
        checkinText.innerText = '✓ Attendance Marked Today';
      } else {
    ...
    if (targetSection) {
      ...
      switch (viewName) {
        ...
        case 'projects':
          ...
          break;
        case 'directory':
          Directory.render(targetSection, App.showToast);
          break;
        case 'analytics':
          Analytics.render(targetSection);
          break;
        case 'settings':
          Settings.render(targetSection, App.showToast);
          break;
      }
    }
  }
  ...
  static bindGlobalEvents() {
    // Hash Change Listener for URL Deep Linking (e.g. #profile?id=ODHR20210002)
    ...
    document.querySelectorAll('.nav-tab-item').forEach(item => {
      item.addEventListener('click', (e) => {
        const rawView = e.currentTarget.getAttribute('data-view') || e.currentTarget.getAttribute('data-tab');
        const view = rawView === 'me' ? 'profile' : (rawView === 'employees' ? 'directory' : (rawView === 'reports' ? 'analytics' : rawView));
        App.openView(view);
      });
    });
    ...
    // Password Show/Hide Toggle Button Delegated Listener
    document.addEventListener('click', (e) => {
      const toggleBtn = e.target.closest('.btn-toggle-pw-inside, .btn-toggle-pw-square, .btn-toggle-pw');
      if (toggleBtn) {
        e.preventDefault();
        e.stopPropagation();
        ...
        if (input) {
          ...
          const openSvg = toggleBtn.querySelector('.eye-open');
          const closedSvg = toggleBtn.querySelector('.eye-closed');
    ...
    // Quick Demo Autofill Pills Handler
    document.getElementById('btn-demo-hr1')?.addEventListener('click', () => {
      const emailInput = document.getElementById('signin-email-input');
      const passInput = document.getElementById('signin-password');
      if (emailInput && passInput) {
        emailInput.value = 'hr.eleanor@dayflow.com';
        passInput.value = 'password123';
        App.showToast('Autofilled HR Eleanor credentials!', 'info');
      }
    });

    document.getElementById('btn-demo-hr2')?.addEventListener('click', () => {
      const emailInput = document.getElementById('signin-email-input');
      const passInput = document.getElementById('signin-password');
      if (emailInput && passInput) {
        emailInput.value = 'hr.marcus@dayflow.com';
        passInput.value = 'password123';
        App.showToast('Autofilled HR Marcus credentials!', 'info');
      }
    });
    ...
    document.getElementById('btn-demo-emp')?.addEventListener('click', () => {
      ...
      const passInput = document.getElementById('signin-password');
      if (emailInput && passInput) {
        emailInput.value = 'sarah@dayflow.com';
        passInput.value = 'password123';
        App.showToast('Autofilled Employee Sarah credentials!', 'info');
      }
    });
    ...
    document.getElementById('form-signin')?.addEventListener('submit', (e) => {
      e.preventDefault();
      const errBox = document.getElementById('signin-error-msg');
    ...
    document.getElementById('form-portal-auth')?.addEventListener('submit', (e) => {
      ...
      try {
        ...
        // SECURITY RULE: Restrict regular employees from accessing HR Admin Portal
        if (targetMode === 'admin' && user.role !== 'admin') {
          Auth.logout();
          throw new Error(`🚫 Access Denied: User '${user.name}' (${user.id}) is an Employee. Admin portal is restricted to HR Administrators.`);
        }
    ...
    // Dropdown Item: Check IN / Check OUT Systray Toggle (Strict 1-time per day rule)
    document.getElementById('dropdown-checkin-toggle')?.addEventListener('click', (e) => {
      ...
      // Rule 1: If user already checked IN AND checked OUT for today -> Block repeat check in!
      if (todayRecord && todayRecord.checkIn !== '-' && todayRecord.checkOut !== '-') {
        App.showToast('⚠️ You have already completed your Check-IN and Check-OUT for today. Only 1 attendance punch cycle is allowed per day!', 'warning');
        return;
      }

      // Rule 2: If user is currently checked IN (working) -> Perform Check OUT
      if (todayRecord && todayRecord.checkIn !== '-' && todayRecord.checkOut === '-') {
        todayRecord.checkOut = timeNow;
        todayRecord.workHours = calculateWorkHours(todayRecord.checkIn, timeNow);
        db.updateAttendanceRecord(todayRecord);
        App.showToast(`Checked OUT successfully at ${timeNow}! Calculated Work Hours: ${todayRecord.workHours}.`, 'info');
      } else {
        // Rule 3: Perform initial Check IN for today
        if (todayRecord) {
    ...
    const savedTheme = localStorage.getItem('odoo_theme') || 'light';
...
// Bootstrap App safely (handles ES module deferred load timing)
