...
import { db, calculateWorkHours } from './db.js';
...
export class Attendance {
  static render(container, showToast) {
    ...
    container.querySelector('#btn-toggle-checkin')?.addEventListener('click', () => {
      ...
      if (!todayRecord) {
        ...
        todayRecord.workHours = calculateWorkHours(todayRecord.checkIn, todayRecord.checkOut);
  ...
  static renderAttendanceRows(list, isAdmin, user) {
    ...
    return list.map(att => {
      ...
      let displayWorkHours = att.workHours;
      if (att.checkOut !== '-' && displayWorkHours === 'In Progress') {
         displayWorkHours = calculateWorkHours(att.checkIn, att.checkOut);
      }
