...
export const calculateWorkHours = (checkIn, checkOut) => {
  if (!checkIn || checkIn === '-' || !checkOut || checkOut === '-') return 'In Progress';
  ...
  if (inMins === null || outMins === null) return 'Invalid Time';

  let diff = outMins - inMins;
  if (diff < 0) diff += 24 * 60; 

  const diffHrs = Math.floor(diff / 60);
  const diffMinsRem = diff % 60;
  return `${diffHrs}h ${diffMinsRem}m`;
};
