...
const SEED_ATTENDANCE = [
  ...
  { id: 'att-2', userId: 'ODHR20210001', hrId: 1, date: '2026-08-22', checkIn: '08:55 AM', checkOut: '-', workHours: 'In Progress', status: 'Present' },
...
// Helper: Format Time in standard 12-hour AM/PM format (e.g. "09:00 AM", "05:30 PM")
export function formatTime12h(dateObj = new Date()) {
...
// Helper: Calculate Exact Work Hours dynamically from Check-In and Check-Out time strings
export function calculateWorkHours(checkInStr, checkOutStr) {
  ...
  function parseTimeToMinutes(timeStr) {
    ...
    const timeDigits = cleanStr.replace(/[^0-9:]/g, '');
  ...
  if (diffMins < 0) diffMins += 24 * 60; // Overnight shift handling
...
export class DB {
  ...
  init() {
    ...
    if (!existing || existing === '[]' || !existing.includes('ODHR20210001')) {
  ...
  // HR Managers Methods
  getHRs() {
    let hrs = JSON.parse(localStorage.getItem(STORAGE_KEYS.HRS) || 'null');
    if (!hrs || !Array.isArray(hrs) || hrs.length === 0) {
      localStorage.setItem(STORAGE_KEYS.HRS, JSON.stringify(SEED_HRS));
      hrs = SEED_HRS;
    }
    return hrs;
  ...
  addAttendanceRecord(record) {
    ...
    } else if (record.checkIn && record.checkIn !== '-' && (!record.checkOut || record.checkOut === '-')) {
